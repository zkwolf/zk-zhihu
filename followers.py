...
def get_followers():
    ...
    r = s.get('http://www.zhihu.com/people/zkwolf10824/followers')
    data = r.text
    ...
    for follower in soup.select('h2.zm-list-content-title a'):
        bitch.append(follower.attrs.get('href'))
    '''
        获取所有除第一页以外的所有关注
    '''
    offsets = []
    ...
    header = {
        ...
        'Referer': 'http://www.zhihu.com/people/zkwolf10824/followers',
    ...
    for index in range(divi + 1):
        ...
        request_url = "http://www.zhihu.com/node/ProfileFollowersListV2"
        x = s.post(request_url, data=payload, headers=header)
        temp = json.loads(x.text)
        _follower2 = temp.get('msg')
        follower2 = ''.join(_follower2)
        soup = BeautifulSoup(follower2, 'html.parser')
        for follower in soup.select('h2.zm-list-content-title a'):
            bitch.append(follower.attrs.get('href'))
    '''
        写入txt文件，用来后续分析
